...
$("#logReq").click(function () {
    ...
    if($("#phone").val() == '' && $("#tm").val() == ''){
        ...
        $.ajax({
            type: "POST",
            url: "http://120.27.208.192:1012/api/h5/Member/Query",
            contentType: "application/json", //必须有
            data: JSON.stringify({ 'Moblie':'', 'Nick':$("#tm").val(),'OpenId':openid}),
                ...
                success:function(data){
                    if(data==1){//登录成功
                        $.ajax({
                            ...
                            url: "http://120.27.208.192:1012/api/h5/Member/Query",
                            contentType: "application/json", //必须有
                            data: JSON.stringify({ 'Moblie':$("#phone").val(), 'Nick':'','OpenId':openid}),
                            success: function (result) {
                                var res = JSON.parse(result);
                                if(res.StatusCode == 'S01'){
                                    $("#bg").hide();
...
$("#regReq").click(function () {
    ...
    if($("#phone2").val() == ''){
        ...
        //           $.ajax({
        //               type: "POST",
        //               url: "http://120.27.208.192:1012/api/h5/Member/Register",
        //               contentType: "application/json", //必须有
        ...
        $.ajax({
            ...
            url: "http://120.27.208.192:1012/api/h5/Member/Register",
            contentType: "application/json", //必须有
            data: JSON.stringify({ 'Moblie':$("#phone2").val(),'RecommendMoblie':RecommendMoblie,'OpenId':$("#phone2").val(),'RecommendNick':RecommendedNick,'Source':Request['source'],'RegistTime':resTime}),
            success: function (result) {
                var res = JSON.parse(result);
                if(res.StatusCode == 'S01'){
                    $("#bg2").hide();
    ...
    var time = setInterval(function () {
        ...
        if(timeN==0){
            ...
            if(jifen && getQueryString('user')){
                $.ajax({
                    type: "POST",
                    url: "http://120.27.208.192:1012/api/h5/Member/Award_point",
                    contentType: "application/json", //必须有
                    ...
                    success: function (result) {
                        ...
                        var res = JSON.parse(result);
                        if(res.StatusCode == 'S01'){
                            alert("推荐人"+getQueryString('user')+"积分：200")
                        }
                    }
                });
            }
            ...
            if(jifen && Request['phone']){
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        var res = JSON.parse(result);
                        if(res.StatusCode == 'S01'){
                            alert("推荐人"+Request['phone']+"积分：200")
                        }
                    }
                });
            }
...
var u_r_l = "http://www.createcdigital.com/createc-new/levis/share.html?source="+Request['source'];
if(localStorage.getItem("moble")){
...
function share(url) {
    wx.ready(function () {
        wx.onMenuShareTimeline({
            title: "Levi's要带我去旧金山玩了，你也想去？", // 分享标题
        ...
        wx.onMenuShareAppMessage({
            title: "Levi's要带我去旧金山玩了，你也想去？", // 分享标题
            desc: "要跟着Levi's去旧金山玩的人，是你吗？", // 分享描述
            link: url, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
            imgUrl: 'http://www.createcdigital.com/createc-new/levis/img/logo.jpg', // 分享图标
